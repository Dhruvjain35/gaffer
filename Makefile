...
deploy:
	gcloud run deploy gaffer --source . --region us-central1 --allow-unauthenticated \
	  --set-env-vars "GOOGLE_GENAI_USE_VERTEXAI=1,GOOGLE_CLOUD_PROJECT=$(GOOGLE_CLOUD_PROJECT),GOOGLE_CLOUD_LOCATION=global,GEMINI_MODEL=gemini-3.5-flash,PHOENIX_API_KEY=$(PHOENIX_API_KEY),PHOENIX_COLLECTOR_ENDPOINT=$(PHOENIX_COLLECTOR_ENDPOINT),PHOENIX_PROJECT_NAME=gaffer"
